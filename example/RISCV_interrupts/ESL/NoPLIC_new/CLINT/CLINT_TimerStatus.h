...
public:
    ...
    void run() {
        while (true) {
            fromTimer_L->master_read(mtimeL_data);
            fromTimer_H->master_read(mtimeH_data);

            fromMemory_timecmp_L->slave_read(mtimecmpL_data);
            fromMemory_timecmp_H->slave_read(mtimecmpH_data);

            if ((mtimeL_data >= mtimecmpL_data) && (mtimeH_data >= mtimecmpH_data)) {
                MTIP_port->slave_write(true);
            } else {
                MTIP_port->slave_write(false);
            }

            wait(WAIT_TIME, SC_PS);
//            wait(SC_ZERO_TIME);//FIXME: do I really need this?
        }
    }
};

-- RTL MAPPING --
macro state_04  		: boolean 	:= CU/section = IF_writeME and CU/dmemAccess and CU/CUtoME_port.req = ME_WR				end macro;
macro state_05  		: boolean 	:= CU/section = IF_readME  and CU/dmemAccess and CU/CUtoME_port.req = ME_WR				end macro;
macro state_11			: boolean 	:= CU/section = IF_writeME and CU/dmemAccess and CU/CUtoME_port.req = ME_RD 				end macro;
macro state_12 			: boolean 	:= CU/section = IF_readME  and CU/dmemAccess and CU/CUtoME_port.req = ME_RD				end macro;
macro state_16			: boolean 	:= CU/section = IF_writeME and not(CU/dmemAccess)							end macro;
macro state_17  		: boolean 	:= CU/section = IF_readME  and not(CU/dmemAccess) 							end macro;
...
macro R_ENC   			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_R   							end macro;
macro I_I_ENC 			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_I_I 							end macro;
macro I_L_ENC 			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_I_L 							end macro;
macro I_J_ENC 			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_I_J 							end macro;
macro S_ENC   			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_S   							end macro;
macro B_ENC   			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_B   							end macro;
macro U_ENC   			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_U   							end macro;
macro J_ENC   			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_J  							end macro;
macro ERR_ENC 			: boolean 	:= getEncType(getOpcode(CU/loadedInst)) = ENC_ERR 							end macro;
...
macro rs2Num 			: unsigned	:= next(getRs2Addr(CU/loadedInst,getOpcode(CU/loadedInst))); 						end macro;
macro nop 			: unsigned 	:= ((loadedData and resize(19,32)) or resize(19,32)) 							end macro;
macro LUI_INSTR 		: boolean 	:= getInstrType(getFunct3(CU/loadedInst),getFunct7(CU/loadedInst),getOpcode(CU/loadedInst)) = INSTR_LUI end macro;

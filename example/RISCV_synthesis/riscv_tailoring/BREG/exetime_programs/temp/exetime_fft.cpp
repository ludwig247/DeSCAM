//
// Created by Paulius Morkunas (paulius.morku@gmail.com) on 30.11.18.
//

#define SAMPLES 1500

const int twiddleFactor[192] = {
	     0,	   402,	   804,	  1205,	  1606,	 2005,	  2404,	  2801,	  3196,	  3590,	  3981,	  4370,	  4756,	  5139,	  5519,	  5896,
	  6270,	  6639,	  7005,	  7366,	  7723,	 8075,	  8423,	  8765,	  9102,	  9433,	  9759,	 10079,	 10393,	 10701,	 11002,	 11297,
	 11585,	 11865,	 12139,	 12405,	 12664,	 12915,	 13159,	 13394,	 13622,	 13841,	 14052,	 14255,	 14449,	 14634,	 14810,	 14977,
	 15136,	 15285,	 15425,	 15556,	 15678,	 15790,	 15892,	 15985,	 16068,	 16142,	 16206,	 16260,	 16304,	 16339,	 16363,	 16378,
	 16383,	 16378,	 16363,	 16339,	 16304,	 16260,	 16206,	 16142,	 16068,  15985,	 15892,	 15790,	 15678,	 15556,	 15425,	 15285,
	 15136,	 14977,	 14810,	 14634,	 14449,	 14255,	 14052,	 13841,	 13622,	 13394,	 13159,	 12915,	 12664,	 12405,	 12139,	 11865,
	 11585,	 11297,	 11002,	 10701,	 10393,	 10079,	  9759,	  9433,	  9102,	  8765,	  8423,	  8075,	  7723,	  7366,	  7005,	  6639,
      6270,	  5896,	  5519,	  5139,	  4756,	  4370,	  3981,	  3590,	  3196,	  2801,	  2404,	  2005,	  1606,	  1205,	   804,	   402,
	     0,	  -402,	  -804,  -1205,	 -1606,	 -2005,	 -2404,	 -2801,	 -3196,  -3590,  -3981,	 -4370,	 -4756,	 -5139,	 -5519,	 -5896,
	 -6270,	 -6639,	 -7005,	 -7366,	 -7723,	 -8075,	 -8423,	 -8765,	 -9102,	 -9433,	 -9759, -10079,	-10393,	-10701,	-11002,	-11297,
	-11585,	-11865, -12139,	-12405,	-12664,	-12915,	-13159,	-13394,	-13622,	-13841,	-14052,	-14255,	-14449,	-14634,	-14810,	-14977,
	-15136,	-15285,	-15425, -15556,	-15678,	-15790,	-15892,	-15985,	-16068,	-16142,	-16206,	-16260,	-16304,	-16339,	-16363,	-16378,
};

const unsigned char revTable[256] = {
    ,
};

int inputRE[SAMPLES];
int inputIM[SAMPLES];

void fft_16bit(int *inputRE, int *inputIM)
{
    int twCos, twSin;                    	// complex components of twiddle factor
    int tempRE, tempIM;						// temporary result complex pair
    int shiftRE, shiftIM;

    int s = 1;
    unsigned char k = 7;

    int i, j, m, istep;

    // Bit reverse the order of the inputs.
    for (i = 1; i < 240; i++)
    {
        if (i < revTable[i])
        {
        	tempRE = inputRE[i];
        	inputRE[i] = inputRE[revTable[i]];
        	inputRE[revTable[i]] = tempRE;
        }
    }

    while (s <= 255) {
    	istep = s << 1;
        for (m = 0; m < s; ++m) {
            j = m << k;
            twCos =  twiddleFactor[j+64];
            twSin = -twiddleFactor[j];
            for (i = m; i < 256; i += istep)
            {
            	j = i + s;
            	tempRE = ((twCos * inputRE[j]) >> 16) - ((twSin * inputIM[j]) >> 16);
            	tempIM = ((twCos * inputIM[j]) >> 16) + ((twSin * inputRE[j]) >> 16);
                shiftRE = inputRE[i]>>1;
                shiftIM = inputIM[i]>>1;
                inputRE[j] = shiftRE - tempRE;
                inputIM[j] = shiftIM - tempIM;
                inputRE[i] = shiftRE + tempRE;
                inputIM[i] = shiftIM + tempIM;
            }
        }
        k--;
        s = istep;
    }
}

int main(int argc, char** arg) {
    for (int i = 0; i < SAMPLES/2; i++) {
        inputRE[i] = i;
        inputRE[SAMPLES - 1 - i] = i;
	}

	fft_16bit(inputRE, inputIM);

    return 0;
}


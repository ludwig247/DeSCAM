...
public:
    ...
    slave_in<ISAtoRF_IF> ISAtoRF_port;
    slave_out<RFtoISA_IF> RFtoISA_port;
...
#pragma clang diagnostic ignored "-Wmissing-noreturn"
    while (true) {
        ...
        requestReceived = ISAtoRF_port->nb_read(ISAtoRF_data);

        if (requestReceived) {
            if (ISAtoRF_data.rwReq == RF_WR) {
...
#ifdef LOGTOFILE
                    cout << "S5: @RF: Writing to register x" << dec << ISAtoRF_data.dst << " = 0x" << hex
                         << ISAtoRF_data.dstData << " = " << dec << ISAtoRF_data.dstData << "(dec)" << endl;
                    log();
#endif
                }
            } else {
                RFtoISA_data.data = readRegfile(ISAtoRF_data.dst,
                                                reg_file_01,
                                                reg_file_02,
                                                reg_file_03,
                                                reg_file_04,
        ...
        // return current data
        RFtoISA_port->nb_write(RFtoISA_data);
    }

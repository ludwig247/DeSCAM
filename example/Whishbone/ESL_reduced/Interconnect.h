    ...
    void fsm() {
        while (true) {
            ...
            if (section == START) {
                ...
                if (from_master.addr == SLAVE0) {
                    slave_out0->set(from_master);
                    slave_number = 0;
                } else if (from_master.addr== SLAVE1) {
                    slave_out1->set(from_master);
                    slave_number = 1;
                } else if (from_master.addr == SLAVE2) {
                    slave_out2->set(from_master);
                    slave_number = 2;
                } else if (from_master.addr == SLAVE3) {
                    slave_out3->set(from_master);
                    slave_number = 3;
                } else {
                    to_master.ack = true;
                    to_master.data = false;
                    to_master.err = false;
                    nextsection = DONE;
                }
            }
            if (section == TRANSMITTING) {
                //nextsection = DONE;
                from_master.data = false;
                from_master.addr = SLAVE0;

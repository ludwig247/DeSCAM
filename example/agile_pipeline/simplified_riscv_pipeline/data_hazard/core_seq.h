...
public:
    ...
    enum Sections {
        SEC_IF, SEC_ID, SEC_EX, SEC_ME, SEC_WB
    };
    ...
    void fsm() {
        while (true) {
            ...
            if (section == SEC_IF) {
                ...
                // Set signals for the next stage
                encodedInstr = MEtoCO_data.loadedData;
                PCReg = PCReg + 4;

                nextsection = SEC_ID; // Move on to the next stage
            } else if (section == SEC_ID) {
                // Set signals for the next stage
...
#endif
                nextsection = SEC_EX; // Move on to the next stage
            } else if (section == SEC_EX) {
                if (ALUSrc == ALUS_RS2)
                    ALUResult = getALUResult(ALUFunc, RS1, RS2);
                else if (ALUSrc == ALUS_IMM)
                    ALUResult = getALUResult(ALUFunc, RS1, imm);
                else
                    ALUResult = UNDEFINED_VALUE;
                
#ifndef SC_SIMULATE
                dummyout->write(false); // Dummy write to force wait state
#endif
                nextsection = SEC_ME; // Move on to the next stage
            } else if (section == SEC_ME) {
                if (memAction == MA_READ) {
                    COtoME_data.req = ME_RD;
                    COtoME_data.mask = MT_W;
                ...
                nextsection = SEC_WB; // Move on to the next stage
                ...
                dummyout->write(false); // Dummy write to force wait state
#endif
                nextsection = SEC_IF; // Move on to the next stage
            }
        }
    }
};

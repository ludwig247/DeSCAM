...
macro toRegsPort_notify 		: boolean 	:= 
	if   (fetch_16 and prev(rst)) then false
	elsif(fetch_16 and prev(fetch_16)) then false
	elsif(fetch_17 and prev(fetch_17)) then false
	elsif(execute_12 and prev(execute_12)) then false
	elsif(execute_11 and prev(execute_11)) then false
	elsif(execute_4 and prev(execute_4)) then false
	elsif(execute_5 and prev(execute_5)) then false
	elsif(fetch_16 and prev(execute_5)) then false
	elsif(execute_5 and prev(execute_4)) then false
	elsif(execute_12 and prev(execute_11)) then false
	elsif(fetch_16 and prev(execute_12)) then true
	elsif(fetch_17 and prev(fetch_16)) then false
	elsif(prev(fetch_17) and prev(getEncType(CU/MEtoCU_port.loadedData)) = ENC_I_L) then false 
	elsif(prev(fetch_17) and prev(getEncType(CU/MEtoCU_port.loadedData)) = ENC_S) then false 
	else next(CU/CUtoRF_port_notify,2) 
	end if; 
end macro;
...
macro toRegsPort_sig_dst 	  	: unsigned 	:= next(CU/CUtoRF_port.dst,2)     end macro;
macro toRegsPort_sig_dstData      	: unsigned 	:= next(CU/CUtoRF_port.dstData,2) end macro;
...
macro regfileWrite_dst       	: unsigned 	:= next(CU/CUtoRF_port.dst,2) end macro;
macro regfileWrite_dstData      : unsigned 	:= next(CU/CUtoRF_port.dstData,2) end macro;

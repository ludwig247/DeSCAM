...
prove:
	 at t+1: fetch_16;
	 at t+1: memoryAccess_addrIn = resize(0,32);
	 at t+1: memoryAccess_dataIn = resize(0,32);
	 at t+1: memoryAccess_mask = MT_W;
	 at t+1: memoryAccess_req = ME_RD;
	 at t+1: pcReg = resize(0,32);
	 at t+1: temp_dst = resize(0,32);
	 at t+1: toMemoryPort_sig_addrIn = resize(0,32);
	 at t+1: toMemoryPort_sig_dataIn = resize(0,32);
	 at t+1: toMemoryPort_sig_mask = MT_W;
	 at t+1: toMemoryPort_sig_req = ME_RD;
...
local_determination_requirements:
  at t+1: if(CU/CUtoME_port_notify) then determined(CU/CUtoME_port) end if;
end property;
...
right_hook:
    t+1;
local_determination_requirements:
  at t+1: determined(CU/CUtoME_port);
end property;

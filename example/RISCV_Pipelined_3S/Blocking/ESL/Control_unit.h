...
public:
    ...
    enum Sections {
        instrFetch, instrDecode, readRegfile, execute, writeBack, setPipelineSignals, updatePC, memAccess
    };
...
void Control_unit::run() {
    ...
    while (true) {
        ...
        if (section == instrDecode) {
            ...
            if (DEtoCU_data.encType_s1 == ENC_I_J || DEtoCU_data.encType_s1 == ENC_B || DEtoCU_data.encType_s1 == ENC_J) {
                cout << "S2: @DE: Immediate = 0x" << hex << DEtoCU_data.imm_s1 << "(hex) = "
                     << dec << DEtoCU_data.imm_s1 << "(dec)" << endl;
            } else {
        ...
        if (section == updatePC) {
            ...
            if (DEtoCU_data.encType_s1 == ENC_I_L || DEtoCU_data.encType_s1 == ENC_S) {

#ifndef RISCV_RI5CY_H_
#define RISCV_RI5CY_H_
        ...
        if (section == Sections::FETCH_PH){
            ...
            if(getInstrType(encodedInstr) != InstrType::INSTR_UNKNOWN){
                if (getEncType(encodedInstr) == ENC_R){
                    ...
                    aluResult = getEncUALUresult(encodedInstr, pcIf);
...
InstrType ISA_ri5cy::getInstrType(unsigned int encodedInstr) const {
...
unsigned int ISA_ri5cy::getRs1Addr(unsigned int encodedInstr) const {
...
unsigned int ISA_ri5cy::getRs2Addr(unsigned int encodedInstr) const {
...
unsigned int ISA_ri5cy::getRdAddr(unsigned int encodedInstr) const {
...
unsigned int ISA_ri5cy::getImmediate(unsigned int encodedInstr) const {
...
ALUfuncType ISA_ri5cy::getALUfunc(InstrType instr) const {
...
ME_MaskType ISA_ri5cy::getMemoryMask(InstrType instr) const {
...
unsigned int ISA_ri5cy::readRegfile(unsigned int src, RegfileType regfile) const {
...
unsigned int ISA_ri5cy::getALUresult(ALUfuncType aluFunction, unsigned int operand1, unsigned int operand2) const {
...
unsigned int ISA_ri5cy::getEncUALUresult(unsigned int encodedInstr, unsigned int pcReg) const {

    if (getInstrType(encodedInstr) == InstrType::INSTR_LUI) {
        return IMM_U_FIELD(encodedInstr);
    } else { // InstrType::INSTR_AUIPC)
        return IMM_U_FIELD(encodedInstr) + pcReg;
    }
}

#endif //RISCV_RI5CY_H_

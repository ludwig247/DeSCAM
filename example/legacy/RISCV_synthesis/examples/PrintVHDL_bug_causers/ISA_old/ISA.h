...
void ISA::run() {
    ...
    while (true) {
        ...
        if (section == Sections::fetch) {
...
#if !defined(NDEBUG) && defined(ISA_DEBUG)
            cout << endl << "==========================================================================" << endl;
            ...
            if (encodedInstr.loadedData == 0x0) {
#if !defined(NDEBUG) && defined(ISA_DEBUG)
        ...
        if (section == Sections::setControl) {
            if (decodedInstr.encType == I) {
                if (decodedInstr.instrType == InstrType::addI ||
                    decodedInstr.instrType == InstrType::sltI ||
                    decodedInstr.instrType == InstrType::sltIu ||
                    decodedInstr.instrType == InstrType::xorI ||
                    decodedInstr.instrType == InstrType::orI ||
                    decodedInstr.instrType == InstrType::andI ||
                    decodedInstr.instrType == InstrType::sllI ||
                    decodedInstr.instrType == InstrType::srlI ||
...
#if !defined(NDEBUG) && defined(ISA_DEBUG)
            cout << "@ALU: Decoded ALU operands: OP1 = 0x" << hex << alu_op1 << " = " << dec << alu_op1
                 << "(dec), OP2 = 0x" << hex << alu_op2 << " = " << dec << alu_op2 << "(dec)" << endl;
#endif
...
#if !defined(NDEBUG) && defined(ISA_DEBUG)
            cout << "@ALU: Output = 0x" << hex << ALUtoCtl_data.ALU_result << " = "

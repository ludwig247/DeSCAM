...
parser.add_argument('-o', dest='outputDirESL', default=(os.getcwd() + '/generated/ESL/'), metavar='dir', help='Output directory for generated files')
parser.add_argument('-ortl', dest='outputDirRTL', default=(os.getcwd() + '/generated/RTL/'), metavar='dir', help='Output directory for environmental RTL files')
...
if not os.path.exists(args.outputDirESL):
    os.makedirs(args.outputDirESL)

if not os.path.exists(args.outputDirRTL):
    os.makedirs(args.outputDirRTL)
...
instructionFieldInfo = [
        ...
        [OPCODE_I_S, '0x0', '0x09', 'INSTR_SFENCE.VMA', 'RS1', 'RS2', 'RD'],
        # CSR instructions
...
usedEncTypes = list()
usedEncTypes.append('ENC_X') # ENC_X encryption is needed to allow ignoring of unsupported instructions
for instr in usedInstrTypes:
...
outputISA = open(args.outputDirESL + '/ISA.h','w+')
...
for enc in usedEncInstrTypes:
    if enc[0:6] == 'ENC_I_':
        if 'ENC_I_*' not in usedImmediateEncTypes:
...
outputCpuInterfaces = open(args.outputDirESL + '/CPU_Interfaces.h','w+')
...
outputRegs = open(args.outputDirESL + '/regs.h','w+')
outputRegs.write('\
...
\n\
    slave_in<RegfileWriteType> ISAtoRF_port;\n\
    slave_out<RegfileType> RFtoISA_port;\n\
...
outputRegs.write('\
\t\tRFtoISA_port->nb_write(regfile);\n\
\t\treq = ISAtoRF_port->nb_read(regfileWrite);\n\
\t\tif (req) {\n')
...
if args.interfacesInclude == '':
    copyfile(args.modulesDir + '/ESL/Memory_Interfaces.h', args.outputDirESL + '/Memory_Interfaces.h')

if csrUsedFlag:
    # Copy ESL modules
    copyfile(args.modulesDir + '/ESL/core_csr.h', args.outputDirESL + '/Core.h')
    copyfile(args.modulesDir + '/ESL/mip.h', args.outputDirESL + '/mip.h')
    copyfile(args.modulesDir + '/ESL/CLINT.h', args.outputDirESL + '/CLINT.h')
    copyfile(args.modulesDir + '/ESL/bus_csr.h', args.outputDirESL + '/bus.h')
    # Copy RTL modules
    copyfile(args.modulesDir + '/RTL/core_csr.vhd', args.outputDirRTL + '/Core.vhd')
    copyfile(args.modulesDir + '/RTL/mip.vhd', args.outputDirRTL + '/mip.vhd')
    copyfile(args.modulesDir + '/RTL/bus_csr.vhd', args.outputDirRTL + '/bus.vhd')
    copyfile(args.modulesDir + '/RTL/env_module_types.vhd', args.outputDirRTL + '/env_module_types.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT_TimerStatus.vhd', args.outputDirRTL + '/CLINT_TimerStatus.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT_Timer.vhd', args.outputDirRTL + '/CLINT_Timer.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT_TimerStatus.vhd', args.outputDirRTL + '/CLINT_TimerStatus.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT_SoftwareStatus.vhd', args.outputDirRTL + '/CLINT_SoftwareStatus.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT_Memory.vhd', args.outputDirRTL + '/CLINT_Memory.vhd')
    copyfile(args.modulesDir + '/RTL/CLINT.vhd', args.outputDirRTL + '/CLINT.vhd')
else:
    # Copy ESL modules
    copyfile(args.modulesDir + '/ESL/core_simple.h', args.outputDirESL + '/Core.h')
    # Copy RTL modules
    copyfile(args.modulesDir + '/RTL/core_simple.vhd', args.outputDirRTL + '/Core.vhd')
...
outputModules = open(args.outputDirESL + '/modules.h','w+')

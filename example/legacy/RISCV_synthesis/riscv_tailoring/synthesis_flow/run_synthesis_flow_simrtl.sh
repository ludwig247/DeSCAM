...
function power_estimate()
{
	...
	if !(cp $SCRIPT_DIR/work/results/RTL/${rtlname}/* $SCRIPT_DIR/work/simulation_files/ &&
...
do
	...
	do
		export rtlname=${fulldesign}
		export saifname=${rtlname}_${progname}

		power_estimate
	done

	export rtlname=${progname}
	export saifname=${rtlname}
	
	power_estimate
done


export compname=gclock
############################################################################################################
# Elaborate RTL implementation with Design Compiler
for progpath in $SCRIPT_DIR/analysis_programs/compiled_files/*.asm #$manualdesignlist generatedfull generatedlight 
do
	prognamefull=${progpath##*/}
	export progname=${prognamefull%.asm}

	export saifname=${progname}

	rm -rf dc_work
	echo -e "\e[30;48;5;82mElaborating RISC-V RTL using Design Compiler\e[0m"
	if !(dc_shell -f $SCRIPT_DIR/dc_do_all.tcl) then
		echo -e "\e[41mError!\e[0m"
		exit 1
	fi
done

for compname in #gclock #basic 
do
	############################################################################################################

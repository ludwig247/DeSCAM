...
//            }
        ...
        if (section == Sections::execute) {
            ...
            if (getEncType(encodedInstr) == ENC_R) {
                ...
                //Set-up PC
                pcReg = pcReg + 4;
                mip_isa_Port->read(csrfile.mip); //Read mip status
                csrfile.mcause = updateMCAUSE(csrfile.mstatus, csrfile.mie, csrfile.mip, csrfile.mcause);
                csrfile.mepc = updateMEPC(csrfile.mstatus, csrfile.mie, csrfile.mip, csrfile.mcause, csrfile.mepc,
                                          pcReg);
...
unsigned int ISA::getCSRresult(InstrType instr, unsigned int rs1, unsigned int csr) const {
    if (instr == InstrType::INSTR_CSRRW || instr == InstrType::INSTR_CSRRWI) {
        return rs1;
    } else if (instr == InstrType::INSTR_CSRRS || instr == InstrType::INSTR_CSRRSI) {
        return csr | rs1;
    } else if (instr == InstrType::INSTR_CSRRC || instr == InstrType::INSTR_CSRRCI) {
        return csr & ((-rs1) - 1);
    } else
        return 0;
}

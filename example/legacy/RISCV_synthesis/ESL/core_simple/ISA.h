    ...
    while (true) {
        ...
        if (section == Sections::fetch) {
...
#ifdef SC_SIMULATE
            // Terminate if EBREAK instruction is received
            if (fromMemoryData.loadedData == 0x100073) {
                std::cout << "INFO: Termination instruction: EBREAK" << std::endl;
                sc_stop();
                wait(SC_ZERO_TIME);
            }
#endif
            ...
            if (getEncType(encodedInstr) == ENC_R) {
                ...
                RFtoISA_port->read(fromRegsData); //Read register contents

                // Set-up operands for alu by reading from fromRegsData
                ...
                ISAtoRF_port->write(toRegsData); //Perform write back
...
#ifdef SC_SIMULATE
                if (pcReg % 4 != 0) {
                    std::cout << "ERROR: Incorrect PC value: " << pcReg << std::endl;
                    sc_stop();
                    wait(SC_ZERO_TIME);
                }
#endif
                ...
                if (instructionType == INSTR_SB) {
                    toMemoryData.mask = MT_B; // set memory access mask
                } else if (instructionType == INSTR_SH) {
                    toMemoryData.mask = MT_H; // set memory access mask
                } else if (instructionType == INSTR_SW) {
                    toMemoryData.mask = MT_W; // set memory access mask
                } else {
                    toMemoryData.mask = MT_X; // invalid memory access mask (invalid instruction)
                }
#else
                toMemoryData.mask = getMemoryMask(getInstrType(encodedInstr)); // set memory access mask
#endif
                toMemoryData.addrIn = aluResult; // Set address for stores
                toMemoryData.dataIn = readRegfile(RS2_FIELD(encodedInstr), fromRegsData); // Set data for stores, rs2 = source for store

                COtoME_port->write(toMemoryData); // Request store
                ...
                if (getInstrType(encodedInstr) == INSTR_LUI) {
                    //Set-up operands for alu by reading from fromRegsData
                    aluOp1 = getImmediate(encodedInstr); //TODO assign aluResult directly?
                    aluOp2 = 0;

                    aluResult = getALUresult(ALU_COPY1, aluOp1, 0); //Compute result //TODO: replace by add?
                } else { //INSTR_AUIPC
                    // Set-up operands for alu by reading from fromRegsData
                ...
                toRegsData.dst = RD_FIELD(encodedInstr); //Compute destination
                toRegsData.dstData = aluResult; //Compute result
                ...
                //Set up write back
                toRegsData.dst = RD_FIELD(encodedInstr);
                toRegsData.dstData = pcReg + 4; //Compute result
                ...
                if (instructionType == INSTR_LB) {
                    ...
                    toMemoryData.mask = MT_BU; // set memory access mask
                } else if (instructionType == INSTR_LHU) {
                    toMemoryData.mask = MT_HU; // set memory access mask
                } else {
                    toMemoryData.mask = MT_X; // invalid memory access mask (invalid instruction)
                }
#else
                toMemoryData.mask = getMemoryMask(getInstrType(encodedInstr)); // set memory access mask
#endif
                toMemoryData.addrIn = aluResult; // Set address for loads
                ...
                if (getInstrType(encodedInstr) == INSTR_UNKNOWN) {
                    std::cout << "WARNING: Unrecognized instruction (0x" <<  hex << encodedInstr << ") PC: " << pcReg << std::endl;
                    sc_stop();
                    wait(SC_ZERO_TIME);
                }
#endif
...
unsigned int ISA::readRegfile(unsigned int src, RegfileType fromRegsData) const {
    if (src == 0) {
        ...
        return fromRegsData.reg_file_03;
    } else if (src == 4) {
        return fromRegsData.reg_file_04;
    } else if (src == 5) {
        return fromRegsData.reg_file_05;
    } else if (src == 6) {
        ...
        return fromRegsData.reg_file_28;
    } else if (src == 29) {
        return fromRegsData.reg_file_29;
    } else if (src == 30) {
        return fromRegsData.reg_file_30;
    } else {
        return fromRegsData.reg_file_31;
    }
}

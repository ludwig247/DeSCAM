...
{
	int* parameter;
    // parameter = 0xABCD0000;
	parameter = 0x00006000;
	
    initialize();
	
    Cur_Vertical_Sep = parameter[1];
    High_Confidence = parameter[2];
    Two_of_Three_Reports_Valid = parameter[3];
    Own_Tracked_Alt = parameter[4];
    Own_Tracked_Alt_Rate = parameter[5];
    Other_Tracked_Alt = parameter[6];
    Alt_Layer_Value = parameter[7];
    Alt_Layer_Value &= 0x3;
    Up_Separation = parameter[8];
    Down_Separation = parameter[9];
    Other_RAC = parameter[10];
    Other_Capability = parameter[11];
    Climb_Inhibit = parameter[12];

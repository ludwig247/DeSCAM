...
public:
    ...
    Regs(sc_module_name name) :
            ...
            reg_file_09(0),
            reg_file_10(0),
            reg_file_11(4096), //Berkeley Boot Loader reads this register on startup to learn the DTB base address
            reg_file_12(0),    //It can remain initialized to this on running riscv-tests.
            reg_file_13(0),
            reg_file_14(0),
            reg_file_15(0),
            reg_file_16(0),
...
void Regs::run() {
    ...
    while (true) {
        ...
        reg_ecall_Port->nb_write(regfileECall);

        rec = toRegsPort->nb_read(regfileWrite); // read request

        if (rec) {

            if (regfileWrite.dst != 0 && regfileWrite.exception == 0) {
        ...
        rec = ecall_reg_Port->nb_read(regfileECallWrite); // read request from ecall
        if (rec) {

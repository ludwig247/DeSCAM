...
namespace SCAM {
        ...
        if (nodePeekLhs.isConstTypeNode() && nodePeekRhs.isConstTypeNode()) {
            ...
            if ((nodePeekLhs.nodePeekUnsignedValue() && (nodePeekLhs.nodePeekUnsignedValue()->getValue() == 0)) ||
                ...
                if (nodePeekLhs.nodePeekUnaryExpr() && (nodePeekLhs.nodePeekUnaryExpr()->getOperation() == "-")) {
                    ...
                    // (var * -1) = -var
                    UnaryExpr newUnaryExpr("-", (Expr *) simplifyVisitorLhs.getSimplifiedNode());
                    StmtNodeAlloc stmtNodeAlloc(newUnaryExpr, false);
                    nodePointer = stmtNodeAlloc.getAllocNode();
                }
                localNodeUpdated = true;
    ...
    void SimplifyVisitor::visit(class TimePointOperand &node) {
    ...
    void SimplifyVisitor::visit(class FunctionOperand &node) {
        ...
        for (auto it: node.getParamValueMap()) {
            SimplifyVisitor simplifyVisitor(it.second);
            if (simplifyVisitor.isNodeSimplified()) {
                isSimplified = true;
                newParamValueMap.insert(std::make_pair(it.first, (Expr *) simplifyVisitor.getSimplifiedNode()));
            } else {
                newParamValueMap.insert(std::make_pair(it.first, it.second));
            }
        }
        if (isSimplified) {
            FunctionOperand newFunctionOperand(node.getFunction(), newParamValueMap);
            StmtNodeAlloc stmtNodeAlloc(newFunctionOperand, false);
            nodePointer = stmtNodeAlloc.getAllocNode();
        }
    }
    ...
    void SimplifyVisitor::visit(class Return &node) {
        SimplifyVisitor simplifyVisitor(node.getReturnValue());

        if (simplifyVisitor.isNodeSimplified()) {
            Return newReturn((Expr *) simplifyVisitor.getSimplifiedNode());
            StmtNodeAlloc stmtNodeAlloc(newReturn, false);
            nodePointer = stmtNodeAlloc.getAllocNode();
        }
    }
    ...
    TimePointOperand *SimplifyVisitor::simplifyNode(TimePointOperand &node) {
        SimplifyVisitor simplifyVisitor(&node);
        return (TimePointOperand *) simplifyVisitor.getSimplifiedNode();
    }

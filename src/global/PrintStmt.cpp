...
void DESCAM::PrintStmt::visit(ITE &node) {
  ...
  for (auto stmt: node.getIfList()) {
    ...
    std::string statement_string = PrintStmt::toString(stmt, indentSize, indent);
    this->ss << statement_string;
    if (statement_string.find('\n') == std::string::npos) this->ss << ";";
    this->ss << std::endl;
  }
  indent -= indentSize;
  for (int i = 0; i < indent; ++i) { this->ss << " "; } //add indent
...
void DESCAM::PrintStmt::visit(DESCAM::CompoundExpr &node) {
  useParenthesesFlag = true;
  this->ss << "{";

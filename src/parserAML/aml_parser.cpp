...
#line 392 "aml_parser.ypp" // lalr1.cc:859
                        {
                            ...
                            if (driver.module->isVariable(yystack_[2].value.as<std::string>())) {
                                error(yystack_[2].location, "A variable named " + yystack_[2].value.as<std::string>() + " is already defined");
                                YYERROR;
                            }
                                ...
                                for (auto &&subvar : variable->getSubVarList()) {
                                    ...
                                    if ((*initexpr)->getDataType()->isEnumType()) {
                                        ...
                                        if (!driver.isConstExpr(*initexpr)) {
                                            error(yystack_[1].location, "initialization values must be constant");
                                            YYERROR;
                                        }
...
#line 500 "aml_parser.ypp" // lalr1.cc:859
                        {
                            ...
                            //check name against the names of the already defined ones
                            if (std::find(yylhs.value.as<std::vector<std::string> >().begin(), yylhs.value.as<std::vector<std::string> >().end(),
                                          yystack_[0].value.as<std::string>()) != yylhs.value.as<std::vector<std::string> >().end()) {
...
#line 847 "aml_parser.ypp" // lalr1.cc:859
                        {
                            error(yystack_[0].location, "functions are not yet supported");
                            YYERROR;
                        }
...
#line 889 "aml_parser.ypp" // lalr1.cc:859
                        {
                            error(yystack_[2].location, "functions are not yet supported");
                            YYERROR;
                        }
...
#line 894 "aml_parser.ypp" // lalr1.cc:859
                        {
                            error(yystack_[1].location, "functions are not yet supported");
                            YYERROR;
                        }

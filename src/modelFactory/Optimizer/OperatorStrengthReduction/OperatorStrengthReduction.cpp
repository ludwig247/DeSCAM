...
void DESCAM::OperatorStrengthReduction::visit(struct Arithmetic &node) {
  ...
  auto arithNode = StmtCastVisitor<DESCAM::Arithmetic>(this->newExpr).Get();
  ...
  if (arithNode->getOperation() == "*") {
    if (NodePeekVisitor::isConstTypeNode(arithNode->getLhs()) &&
      ...
      } else if (auto lhs = NodePeekVisitor::nodePeekUnsignedValue(arithNode->getLhs())) {
...
void DESCAM::OperatorStrengthReduction::visit(DESCAM::Ternary &node) {
  if (auto falseExpr = StmtCastVisitor<BoolValue>(node.getFalseExpr()).Get()) {
    if (!falseExpr->getValue()) {
      this->newExpr = new Logical(node.getCondition(), "and", node.getTrueExpr(), node.getStmtInfo());
      hasReduction = true;
    }
  } else if (auto trueExpr = StmtCastVisitor<BoolValue>(node.getFalseExpr()).Get()) {

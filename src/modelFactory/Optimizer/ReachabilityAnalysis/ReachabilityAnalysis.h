    ...
    */
    ...
    public:
        ...
        ReachabilityAnalysis(
                ...
                std::map<std::string, std::set<Expr *>> allVarValuesMap,
                std::set<std::string> variablesThatHaveReadSet);
    ...
    private:
        ...
        std::map<int, std::vector<CfgNode *>> pathsToIfMap;                     //Stores the paths from while to if statement
        std::map<std::string, std::set<Expr *>> assignmentsToIfValuesMap;       //Stores last assignments from while to if statements
        ...
        std::map<std::string, VariableOperand*> substitutionMap;                              //variables to be substituted in each if statement
        ...
        std::vector<std::pair<Return *, std::vector<Expr *>>> returnValueConditionList;
        std::set<std::pair<Return *, std::vector<Expr *>>> toBeDeletedPair;

        void checkAndAddVariableToSubstitutionMap(VariableOperand *varOp);

        std::vector<Expr*> substituteVariablesInsideIfExpressionWithTheirValues(Expr *ExpressionInsideIf);

        void printWarning(Stmt *IfStmt, bool cannotBe);

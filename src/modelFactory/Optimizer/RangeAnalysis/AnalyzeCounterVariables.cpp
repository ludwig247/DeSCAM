...
DESCAM::AnalyzeCounterVariables::AnalyzeCounterVariables(std::set<std::string> counterVariablesSet,
  ...
  for (auto node : this->CFG) {
    if (auto stmt = node.second->getStmt()) {
      if (auto ifStmt = StmtCastVisitor<DESCAM::If>(stmt).Get()) {
        ...
        for (const auto& variable : variablesInStmtFinder.getVariablesInStmtSet()) {
        ...
        while (trueBranchNode != elseNodeId) {
          if (auto stmtInTrueBranch = this->CFG.at(trueBranchNode)->getStmt()) {
            if (auto assignment = StmtCastVisitor<DESCAM::Assignment>(stmtInTrueBranch).Get()) {
            ...
            } else if (auto nestedIfStmt = StmtCastVisitor<DESCAM::If>(stmtInTrueBranch).Get()) {
        ...
        if (StmtCastVisitor<DESCAM::If>(parentNode->getStmt()).Get()) {
            ...
            while (nodeINElseBranchId < this->CFG.at(nodeINElseBranchId)->getSuccessorList()[0]->getId()) {
              if (auto stmtInTrueBranch = this->CFG.at(nodeINElseBranchId)->getStmt()) {
                if (auto assignment = StmtCastVisitor<DESCAM::Assignment>(stmtInTrueBranch).Get()) {
                  ...
                  DESCAM::FindVariablesAndFunctionsInStatement
                      variablesInStmtFinder(nestedIfStmt, std::set<std::string>{});
                  for (const auto& var : variablesInStmtFinder.getVariablesInStmtSet()) {
                  ...
                  if (!this->counterVariablesInIfStmt.empty()) {
                    ...
                    while (nestedTrueBranchNode != nestedElseNodeId) {
                      if (auto stmtInElseBranch = this->CFG.at(nestedTrueBranchNode)->getStmt()) {
                        if (auto assignment = StmtCastVisitor<DESCAM::Assignment>(stmtInElseBranch).Get()) {
                          if (auto varOp = StmtCastVisitor<DESCAM::VariableOperand>(assignment->getLhs()).Get()) {
                            auto varName = varOp->getVariable()->getFullName();
                            if (counterIncrementAssignmentFound.find(varName) !=
                                counterIncrementAssignmentFound.end()) {
  ...
  for (const auto& var : this->counterVariablesSet) {
...
void DESCAM::AnalyzeCounterVariables::addToMarginalValuesMap(const std::string& varName) {

...
//TODO Check. Remove unused includes
...
DESCAM::ModelFactory::ModelFactory(CompilerInstance &ci) :
    ...
    findGlobal_ = std::make_unique<FindGlobal>(tu, _ci);
}
...
void DESCAM::ModelFactory::addFunctions(DESCAM::Module *module, CXXRecordDecl *decl) {
    ...
    for (const auto &function: findFunction.getFunctionReturnTypeMap()) {
        DataType *datatype;
        ...
        DESCAM_ASSERT(if (functionsMap.find(function.first) != functionsMap.end())
                          new_function = new Function(function.first, datatype, paramMap,
                                                      GlobalUtilities::getLocationInfo(
    ...
    for (const auto &function: findFunction.getFunctionMap()) {
...
//void DESCAM::ModelFactory::addGlobalConstants() {
    Logger::setCurrentProcessedLocation(LoggerMsg::ProcessedLocation::GlobalConstants);

    //Find all global functions and variables
    //std::unique_ptr<IFindGlobal> _findGlobal = std::make_unique<FindGlobal>(pDecl, _ci);

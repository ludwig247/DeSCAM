...
bool DESCAM::FindDataFlow::VisitBinaryOperator(clang::BinaryOperator *binaryOperator) {
  ...
  if (pass == 0) {
    ...
    auto findLHS = FindDataFlowFactory::create(lhs, this->module, ci,find_data_flow_, unsigned_flag);
    ...
    auto findRHS = FindDataFlowFactory::create(rhs, this->module, ci,find_data_flow_, unsigned_flag);
    ...
    if (binaryOperator->getOpcode() == clang::BinaryOperator::Opcode::BO_Assign) {
      ...
      if (binaryOperator->getType()->isUnsignedIntegerType()) {
        ...
        auto findRHS2 =FindDataFlowFactory::create(binaryOperator->getRHS(), this->module, ci,find_data_flow_, true);
...
bool DESCAM::FindDataFlow::VisitConditionalOperator(clang::ConditionalOperator *conditionalOperator) {
  ...
  auto findCond = FindDataFlowFactory::create(conditionalOperator->getCond(), this->module, ci,find_data_flow_, false);
  auto condExpr = findCond->getExpr();
  auto findTrue = FindDataFlowFactory::create(conditionalOperator->getTrueExpr(), this->module, ci,find_data_flow_, unsigned_flag);
  auto trueExpr = findTrue->getExpr();
  auto findFalse = FindDataFlowFactory::create(conditionalOperator->getFalseExpr(), this->module, ci,find_data_flow_, unsigned_flag);
...
//                this->pass = 1;
        ...
        if (methodString == "wait") {
          if (memberCallExpr->getNumArgs() == 2) {
            auto firstArgument = FindDataFlowFactory::create(memberCallExpr->getArg(0), this->module, ci,find_data_flow_, false);
            auto secondArgument = FindDataFlowFactory::create(memberCallExpr->getArg(1), this->module, ci,find_data_flow_, false);
        ...
        for (int i = 0; i < memberCallExpr->getNumArgs(); i++) {
          ...
          auto findArgument = FindDataFlowFactory::create(memberCallExpr->getArg(i), this->module, ci,find_data_flow_, false);
    ...
    if (this->lhsExpr != nullptr) {
      ...
      if (PortOperand *operand = dynamic_cast<PortOperand *>(this->lhsExpr)) {
        ...
        auto getArgument = [=](clang::Stmt *stmt) {
          auto findArgument = FindDataFlowFactory::create(stmt, this->module, ci,find_data_flow_, operand->getDataType()->isUnsigned());
        ...
        if (interface->isBlocking()) {
          ...
          if (methodString == "get" && memberCallExpr->getNumArgs() == 1) {
            auto findArgument = FindDataFlowFactory::create(memberCallExpr->getArg(0), this->module, ci,find_data_flow_, false);
            if (findArgument->getExpr() != nullptr) {
            ...
            auto findArgument = FindDataFlowFactory::create(memberCallExpr->getArg(0), this->module, ci,find_data_flow_,
  ...
  //Membercall: obj->
  if (pass == 1) {
    auto findMemberCall= FindDataFlowFactory::create(memberCallExpr, this->module, ci,find_data_flow_, false);
...
bool DESCAM::FindDataFlow::VisitMemberExpr(clang::MemberExpr *memberExpr) {
  ...
  auto findParentOfSubVar = FindDataFlowFactory::create((*memberExpr->child_begin()), this->module, ci,find_data_flow_, false);
...
bool DESCAM::FindDataFlow::VisitUnaryOperator(clang::UnaryOperator *unaryOperator) {
  ...
  if (pass == 0) {
    ...
    auto subExpr = FindDataFlowFactory::create(unaryOperator->getSubExpr(), this->module, ci,find_data_flow_, unsigned_flag);
...
bool DESCAM::FindDataFlow::VisitWhileStmt(clang::WhileStmt *whileStmt) {
  if (pass == 0) {
    ...
    auto conditionStmt = FindDataFlowFactory::create(whileStmt->getCond(), this->module, ci,find_data_flow_, false);
...
bool DESCAM::FindDataFlow::VisitIfStmt(clang::IfStmt *ifStmt) {
  if (pass == 0) {
    ...
    auto conditionStmt = FindDataFlowFactory::create(ifStmt->getCond(), this->module, ci,find_data_flow_, false);
...
bool DESCAM::FindDataFlow::VisitCallExpr(clang::CallExpr *callExpr) {
  ...
  if (globalFunctionMap.find(func_name) != globalFunctionMap.end()) {
    ...
    for (int i = 0; i < callExpr->getNumArgs(); i++) {
      ...
      auto findArgument = FindDataFlowFactory::create(callExpr->getArg(i), this->module, ci,find_data_flow_, false);
      if (findArgument->getExpr() == nullptr)
...
bool DESCAM::FindDataFlow::VisitImplicitCastExpr(clang::ImplicitCastExpr *implicitCastExpr) {
  if (implicitCastExpr->getType()->isUnsignedIntegerType() &&
    ...
    auto unsigendSearch= FindDataFlowFactory::create(implicitCastExpr->getSubExpr(), module, ci,find_data_flow_,
...
bool DESCAM::FindDataFlow::VisitCXXStaticCastExpr(clang::CXXStaticCastExpr *staticCastExpr) {
  ...
  if (staticCastExpr->getType()->isUnsignedIntegerType() &&
    ...
    auto subExpr = FindDataFlowFactory::create(staticCastExpr->getSubExpr(), module, ci,find_data_flow_, false);
    if (subExpr->getExpr() != nullptr) {
...
bool DESCAM::FindDataFlow::VisitReturnStmt(clang::ReturnStmt *returnStmt) {
  ...
  auto returnExpr = FindDataFlowFactory::create(returnStmt->getRetValue(), module, ci,find_data_flow_, false);
  // Collecting statement location information from clang
...
bool DESCAM::FindDataFlow::VisitArraySubscriptExpr(clang::ArraySubscriptExpr *arraySubscriptExpr) {
  ...
  auto array = FindDataFlowFactory::create(arraySubscriptExpr->getLHS(), module, ci,find_data_flow_, false);
  ...
  if (array->getExpr() != nullptr && array->getExpr()->getDataType()->isArrayType()) {
    if (auto varOp = NodePeekVisitor::nodePeekVariableOperand(array->getExpr())) {
      auto findIndex = FindDataFlowFactory::create(arraySubscriptExpr->getIdx(), module, ci,find_data_flow_, false);

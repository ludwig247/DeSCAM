...
private:
    ...
    void relocateOpStmts(const std::map<int, SCAM::State *> &stateMap, Module *module); //! Allocates and simplifies nodes
    ...
    PropertySuite * generatePropertySuite();

    std::map<std::string ,SCAM::Variable*> stateVarMap;
    //SCAM::PropertySuite * propertySuite;

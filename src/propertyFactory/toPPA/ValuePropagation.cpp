...
namespace SCAM {
    ...
    SCAM::ValuePropagation::ValuePropagation(const std::map<std::string, std::vector<SCAM::Path *> > &sectionPathMap,
        ...
        if (node.getLhs()->getDataType()->isCompoundType()) {
            for (auto sub: node.getLhs()->getDataType()->getSubVarMap()) {
                ...
                if (auto compoundVal = NodePeekVisitor::nodePeekCompoundValue(node.getRhs())) {
                    ...
                    this->variableValueMap.at(subvar_name) = val;
                    ...
                    this->variableValueMap.at(subvar_name) = find_or_add_variable(subVar->getOperandName(),subVar);
                } else {
                    throw std::runtime_error("Unsure3");
                }
            }
            ...
            for (auto sub: node.getLhs()->getDataType()->getSubVarMap()) {
                ...
                if (auto compoundVal = NodePeekVisitor::nodePeekCompoundValue(node.getRhs())) {
                    auto val = compoundVal->getValues().at(sub.first);
                    this->variableValueMap.at(subvar_name) =  find_or_add_variable(subvar_name, val);
                } else if (auto variableOperand = NodePeekVisitor::nodePeekVariableOperand(node.getRhs())) {
                    auto subVar = new VariableOperand(variableOperand->getVariable()->getSubVar(sub.first));
                    auto val = find_or_add_variable(subVar->getOperandName(),subVar);
                    this->variableValueMap.at(subvar_name) = val;
                } else {
                    throw std::runtime_error("Unsure2");
                }
            }
        } else {
            node.getRhs()->accept(*this);
...
void SCAM::ValuePropagation::visit(struct Read &node) {
    ...
    if (node.getPort()->getDataType()->isVoid()) {
        ...
        for (auto subVar :  node.getVariableOperand()->getVariable()->getSubVarList()) {
            ...
            this->variableValueMap[name] = new DataSignalOperand(node.getPort()->getDataSignal()->getSubVar(subVar->getName()));
        }
...
void SCAM::ValuePropagation::visit(struct NBWrite &node) {
    ...
    if (node.getPort()->getInterface()->isShared()) {
        ...
        if (ExprVisitor::getUsedOperands(node.getValue()).size() == 1) {
            ...
            if (node.getValue()->getDataType()->isCompoundType() || node.getValue()->getDataType()->isArrayType()) {
                ...
                for (auto subSig: node.getPort()->getDataSignal()->getSubVarList()) {
                    ...
                    find_or_add_variable(subSig->getFullName(), new DataSignalOperand(subSig));
                    this->variableValueMap[subSig->getFullName()] = this->variableValueMap[var->getSubVar(
...
void SCAM::ValuePropagation::visit(SCAM::ArrayOperand &node) {
    node.getIdx()->accept(*this);
    //find_or_add_variable(node.getOperandName(),&node);
...
void SCAM::ValuePropagation::visit(SCAM::Notify &node) {
    throw std::runtime_error("Not implemented");

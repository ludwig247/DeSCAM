...
/*!
 *
 * @param conditions Conditions of the successor operation
 * @param commitments Commitments of the predecessor
 * @param module
 * @return Returns true if the conditions of the successor are SAT under the constraints of the predecessor
 */
const bool SCAM::CleanUpPaths::isVacous(const std::vector<SCAM::Expr *> &conditions, const std::vector<SCAM::Assignment *> &commitments, Module * module) {
    Path path("vacous");

    std::vector<Stmt*> stmtList;
    for(auto assignment: commitments){
        path.addStmt(assignment);
    }
    for(auto cond: conditions){
        path.addStmt(new If(cond));
    }
    //This class propagetes the values through the path
    ValuePropagation valuePropagation(module);
    valuePropagation.propagatePath(&path);

    //Use path->print() to check the layout of the object

    return isPathUnreachable(&path);

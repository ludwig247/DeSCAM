...
OperationPruning::OperationPruning(const std::map<int, State2 *> &stateMap, Module *module) : module(module) {
    struct operationReachability_t {
        Operation2 *op = nullptr;
        bool markedAsReachable = false;
    };

    std::map<State2 *, std::vector<operationReachability_t>> operationReachabilityMap;
    ...
    State2 *initialState = stateMap.at(-1);
    Operation2 *initialOperation = *initialState->getOutgoingOperationsList().begin();
    // create new states in order to keep the original stateMap
    State2 *initialState_new = new State2(*initialOperation->getState());
    State2 *nextState_new = new State2(*initialOperation->getNextState());
    ...
    //new Operation2(*initialOperation, initialState_new, nextState_new);

    std::vector<Operation2 *> reachableOpList1;
    std::vector<Operation2 *> reachableOpList2;

    std::vector<Operation2 *> *reachableOpListPrev = &reachableOpList1;
    std::vector<Operation2 *> *reachableOpListNext = &reachableOpList2;
    ...
    while (newOpsFound) {
        ...
        for (auto &rootOp_it : *reachableOpListPrev) {
            State2 *nextState = rootOp_it->getNextState();
            ...
            for (auto &op_it : potentialOps) {
                ...
                for (auto &commit_it : rootOp_it->getCommitmentsList()) {
                    for (auto &assume_it : op_it.op->getAssumptionsList()) {
...
//                }
                ...
                if (opReached) {
                    ...
                    if (state_it == newStateMap.end()) {
                        ...
                        nextState_new = new State2(*op_it.op->getNextState());
                        newStateMap.insert(std::make_pair(op_it.op->getNextState()->getStateId(), nextState_new));
                    } else {
                        nextState_new = state_it->second;
                    }
                    // Create new operation
                    //new Operation2(*op_it.op, newStateMap.at(op_it.op->getState()->getStateId()), nextState_new);
                }
            }
        }
...
const std::map<int, State2 *> &OperationPruning::getNewStateMap() const {
    return newStateMap;
}

const std::vector<State2 *> &OperationPruning::getUnreachableStates() const {

  ...
  for (auto variable: module->getVariableMap()) {
    ...
    if (variable.second->isArrayElement()) {
      if (usedParents.insert(variable.second->getParent()).second) {
        insertRegister(ss,
                       variable.second->getParent()->getName() + "_signal",
                       getDataTypeWrapper(variable.second->getParent()->getDataType()));
      }
...
std::string PrintSkeleton::getFilenameExtention() {
  switch (language) {
    ...
    case SV:return ".sv";
    default:assert(false && "-E- Unsupported language selected in PrintSkeleton::getFilenameExtention");
  }
}

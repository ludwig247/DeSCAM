...
class MasterDummy extends Module {
	...
	core.dontTouch(cnt_signal_r)
	core.dontTouch(req_signal_r)

	io.bus_req_notify := bus_req_notify_r
	io.bus_resp_notify := bus_resp_notify_r
	io.bus_req := bus_req_r
}

object MasterDummy extends App {
	chisel3.Driver.execute(args, () => new MasterDummy)
}

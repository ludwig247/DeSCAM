...
assume:
	at t: start_state;
	at t: (((((((((state = SLAVE_RESP) and
 not((slave_id = resize(1,32)))) and
 not(val_slave_0_sync)) and
 val_slave_1_sync)
 or
 ((((state = SLAVE_RESP) and
 not((slave_id = resize(0,32)))) and
 not((slave_id = resize(1,32)))) and
 val_slave_0_sync))
 or
 ((((state = SLAVE_RESP) and
 not((slave_id = resize(0,32)))) and
 val_slave_0_sync) and
 not(val_slave_1_sync)))
 or
 ((((state = SLAVE_REQ) and
 not((slave_id = resize(1,32)))) and
 not(out_slave_0_sync)) and
 out_slave_1_sync))
 or
 ((((state = SLAVE_REQ) and
 not((slave_id = resize(0,32)))) and
 out_slave_0_sync) and
 not(out_slave_1_sync)))
 or
 (((((state = SLAVE_REQ) and
 not((slave_id = resize(0,32)))) and
 not((slave_id = resize(1,32)))) and
...
assume:
	...
	at t: (((((((((((true and
 not(fromReset)) and
 not((state /= MASTER_REQ))) and
 not((master_id > resize(1,32)))) and
 (state /= SLAVE_RESP)) and
 not(val_master_0_sync)) and
 val_master_1_sync) and
 not(out_slave_0_sync)) and
 not(out_slave_1_sync)) and
 not(val_slave_0_sync)) and
 val_slave_1_sync)
 or
 (true and
 ...
 or
 (((true and
 (state = MASTER_REQ)) and
 (master_id = resize(0,32))) and
 val_master_0_sync))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))));
prove:
	at t_end: start_state;
	at t_end: fromReset = false;
...
assume:
	at t: start_state;
	at t: ((((((((((((((((not((state = SLAVE_RESP)) and
 not(val_master_0_sync)) and
 not(val_master_1_sync)) and
 not(out_slave_0_sync)) and
 not(out_slave_1_sync)) and
 val_slave_1_sync)
 or
 (((((not((state = SLAVE_RESP)) and
 not(val_master_0_sync)) and
 not(val_master_1_sync)) and
 not(out_slave_0_sync)) and
 not(out_slave_1_sync)) and
...
assume:
 ...
 not(fromReset)) and
 not((state /= MASTER_REQ))) and
 (master_id > resize(1,32))) and
 ((val_master_1_sig_haddr >= resize(0,32)) and
 (val_master_1_sig_haddr < resize(65536,32)))) and
 (state /= SLAVE_RESP)) and
 not((resize(1,32) /= resize(1,32)))) and
 not(val_master_0_sync)) and
 val_master_1_sync) and
 not(out_slave_0_sync)) and
 not(out_slave_1_sync)) and
 ...
 (state /= SLAVE_RESP)) and
 not((resize(1,32) /= resize(1,32)))) and
 not((resize(0,32) = resize(1,32)))) and
 not(val_master_0_sync)) and
 val_master_1_sync) and
 not(out_slave_0_sync)) and
 out_slave_1_sync) and
...
assume:
 ...
 not((state /= MASTER_REQ))) and
 (master_id > resize(1,32))) and
 not(((val_master_1_sig_haddr >= resize(0,32)) and
 (val_master_1_sig_haddr < resize(65536,32))))) and
 (state /= SLAVE_RESP)) and
 not((resize(1,32) /= resize(1,32)))) and
 not(val_master_0_sync)) and
 ...
 (state /= SLAVE_RESP)) and
 not((resize(1,32) /= resize(1,32)))) and
 not((resize(1,32) = resize(0,32)))) and
 not(val_master_0_sync)) and
 val_master_1_sync) and
 out_slave_0_sync) and
 not(out_slave_1_sync)) and
...
prove:
	at t_end: start_state;
	at t_end: fromReset = false;--fromReset_at_t;
	at t_end: master_id = master_id_at_t;
	at t_end: slave_id = slave_id_at_t;
	at t_end: state = state_at_t;
	at t_end: val_master_0_notify = false;
